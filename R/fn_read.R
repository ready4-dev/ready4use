...
#' @description read_import_from_csv() is a Read function that reads an R script into memory. Specifically, this function implements an algorithm to read import from comma separated variables file. The function returns Import (a ready4 submodule).
#' @param file_ref_chr File reference (a character vector)
#' @param is_url_1L_lgl Is url (a logical vector of length one), Default: F
#' @return Import (a ready4 submodule)
...
read_import_from_csv <- function (file_ref_chr, is_url_1L_lgl = F) 
{
    if (is_url_1L_lgl) 
        ...
        stringsAsFactors = F)
    import_r3 <- make_r3_from_csv_tb(read_in_file, ready4use_import_lup)
    return(import_r3)
}
