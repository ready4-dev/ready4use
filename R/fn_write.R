    ...
    purrr::walk(1:length(fl_ids_int), ~{
        if (!(ds_ls$versionState == "DRAFT" & files_tb$file_type_chr[.x] == 
            ...
            write_dv_fl_to_loc(database_ui_chr = ds_url_1L_chr, 
                filename_chr = files_tb$file_chr[.x], repo_file_format = files_tb$ds_file_ext_chr[.x], 
                dataverse_chr = Sys.getenv("DATAVERSE_SERVER"), 
                save_type_chr = "original", dest_path_1L_chr = get_local_path_to_dv_data(save_dir_path_chr = local_dv_dir_1L_chr, 
...
write_to_add_urls_to_dss <- function (ds_url, pkg_dss_tb, pkg_nm_1L_chr = ready4fun::get_dev_pkg_nm()) 
{
    ...
    purrr::walk(pkg_dss_tb, ~{
        data(list = ..1, package = pkg_nm_1L_chr, envir = environment())
