...
## Ingest data
To ingest all the RDS objects in a specified dataverse dataset, use the `ingest` method. The `ingest` method transforms `x` into an object of class `Ready4useRecord`, which contains both the ingested data and a `Ready4useRepos` instance describing the source repositor(y|ies). 
...
The dataset may be more meaningful if variables are labelled using descriptions from the data dictionary. This can be accomplished using the `renew` method.
