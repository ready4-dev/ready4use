...
We now specify where we plan to publish `z` using the `Ready4useRepos` class.

```{r}
y <- Ready4useRepos(dv_nm_1L_chr = "fakes",
                    dv_ds_nm_1L_chr = "https://doi.org/10.7910/DVN/W95KED",
                    dv_server_1L_chr = "dataverse.harvard.edu")
```

We can now upload `z` to our preferred data repository using the `share` method. By default, if more than one data repository was specified in `y`, then the dataverse repository will be preferred when sharing. We can overwrite this default by passing either "prefer_gh" or "all" values to the `type_1L_chr` argument. The Ready4useDyad object is now available for download at https://doi.org/10.7910/DVN/W95KED.
